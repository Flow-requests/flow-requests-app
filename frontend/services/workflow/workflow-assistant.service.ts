...
class WorkflowAssistantService {
  ...
  private setUp() {
    ...
    if (!openRouterToken) {
      alert("You need to setup the OpenRouter token");
      return;
    }
    ...
    this.model = new ChatOpenAI({
      ...
      configuration: {
        apiKey: openRouterToken as string,
  ...
  private getCreateTool() {
    ...
    return tool(
      async ({ userPrompt }: { userPrompt: string }) => {
        ...
        prompt +=
          "- Set to the key 'type' must to have value equal one of nodes listed";
  ...
  async processUserMessage(
    ...
    prompt += "- Set to the key 'type' must to have value 'api'";
